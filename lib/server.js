...
class WebServer {
        ...
        this.adapter.getForeignObject('system.config', (err, config) => {
            ...
            this.adapter.getObject('configuration', (err, config) => {
                if (config && config.native && config.native.title) {
                    this._lovelace2Config = config.native;
                    this._lovelace2Config.hideToolbar = this.config.hideHeader;
                } else {
                    this._lovelace2Config = require('./defaultConfig');
                }
    ...
    _manageSubscribesFromConfig() {
        ...
        if (this._lovelace2Config.views) {
            // iterate trough all objects to get all nested entities Ids
            this._flatJSON(this._lovelace2Config.views, entities);
        }

        // this._lovelace2Config.views && this._lovelace2Config.views.forEach(view =>
    ...
    _getConfig() {
        ...
        return {
            ...
            time_zone: tzone,
            components: [
                'lovelace2', 'conversation', 'history', 'shopping_list'
            ],
            version: VERSION
        };
    }
    ...
    async _init() {
        ...
        // on http://localhost:3000/auth/authorize?response_type=code&client_id=http%3A%2F%2Flocalhost%3A3000%2F&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Flovelace2%3Fauth_callback%3D1&state=eyJoYXNzVXJsIjoiaHR0cDovL2xvY2FsaG9zdDozMDAwIiwiY2xpZW50SWQiOiJodHRwOi8vbG9jYWxob3N0OjMwMDAvIn0%3D
        ...
        this._app.get('/lovelace2/cards/*', (req, res) => this.onCards(req, res));

        this._app.get('/local/custom_ui/*', (req, res) => this.onCustomUI(req, res));
        this._app.get('/lovelace2/local/custom_ui/*', (req, res) => this.onCustomUI(req, res));

        this._app.get('/', (req, res) => res.send(this._renderIndex()));
        this._app.get('/lovelace2/', (req, res) => res.send(this._renderIndex()));
        this._app.get('/lovelace2', (req, res) => res.send(this._renderIndex()));
        this._app.get('/lovelace2/:id', (req, res) => res.send(this._renderIndex()));

        this._app.get('/manifest.json', (req, res) => res.json(this._renderManifest()));
        this._app.get('/lovelace2/manifest.json', (req, res) => res.json(this._renderManifest()));

        this._app.use('/frontend_latest/', express.static(getRootPath()));
        this._app.use('/lovelace2/frontend_latest/', express.static(getRootPath()));

        this._app.use('/static/icons/', express.static(__dirname + '/../assets/icons/'));
        this._app.use('/lovelace2/static/icons/', express.static(__dirname + '/../assets/icons/'));

        this._app.use('/images/', express.static(getRootPath() + 'static/images/'));
        this._app.use('/lovelace2/images/', express.static(getRootPath() + 'static/images/'));

        this._app.use('/static/', express.static(getRootPath()));
        this._app.use('/lovelace2/static/', express.static(getRootPath()));
        ...
        this._app.use('/lovelace2/', express.static(getRootPath()));
        ...
        this.adapter.getObject('configuration', (err, obj) => {
            if (JSON.stringify(obj.native) !== JSON.stringify(config)) {
                ...
                this.adapter.setObject('configuration', obj, err => {
                    ...
                    this._sendUpdate('lovelace2_updated');
                });
            }
    ...
    onObjectChange(id, obj) {
        if (id === this.adapter.namespace + '.configuration') {
            this._lovelace2Config = obj.native;
            this._lovelace2Config.hideToolbar = this.config.hideHeader;
            this._manageSubscribesFromConfig();
